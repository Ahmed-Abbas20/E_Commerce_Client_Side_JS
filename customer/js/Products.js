...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadProducts(filteredProducts = null) {
    ...
    const productsToDisplay =
      filteredProducts !== null ? filteredProducts : storedProducts;
    if (productsToDisplay.length === 0) {
      noProducts(true);
      return;
    } else noProducts(false);
    ...
    productsToDisplay.forEach((product) => {
      ...
      const productHTML = `
                    ...
                    data-category="${product.category}">
                        <img src="${
                          product.image
                        }" class="img-fluid product-thumbnail">${soldOutBadge}
    ...
    function noProducts(s) {
      ...
      if (s) message.style.display = "block";
    ...
    function filterProducts() {
      ...
      if (category.length > 0) {
        filteredProducts = filteredProducts.filter((c) =>
    ...
    function filterEvents() {
      document
        .querySelectorAll('input[type="radio"][name="price"]')
        .forEach((r) => {
      ...
      document
        .querySelectorAll(".color-checkbox, .category-checkbox")
        .forEach((c) => {
